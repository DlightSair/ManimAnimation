...
class Test(Scene):
    ...
    def sin_c(self): 
        sin_rect = Rectangle(height=self.rect.get_height()/2-self.bias_height,
                        width=self.rect.get_width(), color=WHITE,
                        fill_color=BLACK,
                        fill_opacity=0.8,
                        ).to_edge(RIGHT).align_to(self.rect, UP) # Creates rectangle surround sine curve
        
        sin_line = Line(sin_rect.get_left(), sin_rect.get_right(), stroke_opacity=0.6) # x-axis of sine curve
        
        sin_curve = FunctionGraph(lambda x: np.sin(x),
                        x_range=[np.pi/3, 2*np.pi+np.pi/3],
                        color=YELLOW, stroke_width=5) # sine curve
        
        sin_curve.stretch_to_fit_width(sin_rect.get_width()-1).move_to(sin_rect.get_center()) 

        sin_xl = DashedLine([sin_curve.get_start()[0],
                        sin_line.get_center()[1], 0],
                        sin_curve.get_start(),
                        dash_length=0.1, stroke_width=5, color=YELLOW)
    ...
    def cos_c(self):
        cos_rect = Rectangle(height=self.rect.get_height()/2-self.bias_height,
                        width=self.rect.get_width(),
                        color=WHITE,
                        fill_color=BLACK,
                        fill_opacity=0.8).to_edge(RIGHT).align_to(self.rect, DOWN) # Creating Rectangle surround cosine curve
        
        cos_line = Line(cos_rect.get_left(), cos_rect.get_right(), stroke_opacity=0.6) # x-axis

        cos_curve = FunctionGraph(lambda x: np.cos(x),
                        x_range=[np.pi/3, 2*np.pi+np.pi/3],
                        color=BLUE, stroke_width=5) # cosine curve
        
        cos_curve.stretch_to_fit_width(cos_rect.get_width()-1).move_to(cos_rect.get_center())

        cos_yl = DashedLine([cos_curve.get_start()[0],
                            cos_line.get_center()[1], 0],
                            cos_curve.get_start(),
                            dash_length=0.1, stroke_width=5, color=BLUE)
